            ...
            for iface_name, iface_data in interfaces.items():
                ...
                if matched_hostid:
                    ...
                    if edge_key not in edges_set:
                        ...
                        # Determine color based on status and bandwidth
                        # Green = good (up and low latency), Yellow = warning, Red = down
                        if iface_status == "up":
                            # Green if bandwidth is reasonable (< 80% utilization)
                            # Calculate utilization if we have max bandwidth
                            max_bw = iface_data.get("bandwidth_max", 0) / 1000000 if iface_data.get("bandwidth_max") else 1000
                            utilization = (bw_in_mbps + bw_out_mbps) / max_bw * 100 if max_bw > 0 else 0
                            
                            if utilization < 80:
                                edge_color = "#10B981"  # Green - good
                            elif utilization < 95:
                                edge_color = "#F59E0B"  # Yellow - warning
                            else:
                                edge_color = "#EF4444"  # Red - critical
                        else:
                            edge_color = "#DC2626"  # Red - down
                        ...
                        edges.append({
                            ...
                            "to": matched_hostid,
                            "label": edge_label,
                            "title": f"Interface: {iface_name}\nDescription: {iface_data.get('description', '')}\n↓ {bw_in_mbps:.2f} Mbps\n↑ {bw_out_mbps:.2f} Mbps\nStatus: {iface_status}\n{'🟢 Good' if edge_color == '#10B981' else '🟡 Warning' if edge_color == '#F59E0B' else '🔴 Issue'}",
                            "color": edge_color,
