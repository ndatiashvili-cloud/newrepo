...
async def ping_device(
    ...
    ip = ip.strip()
    count = max(1, min(count, 20))  # Limit count to reasonable range
    ...
    if "error" in result:
        logging.getLogger(__name__).warning(f"Ping failed for {ip}: {result.get('error')}")
...
async def traceroute_device(
    ...
    ip = ip.strip()
    max_hops = max(1, min(max_hops, 64))  # Limit hops to reasonable range
    ...
    if "error" in result:
        logging.getLogger(__name__).warning(f"Traceroute failed for {ip}: {result.get('error')}")
