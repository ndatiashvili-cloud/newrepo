...
export const zabbixAPI = {
  ...
  // Zabbix metrics endpoints
  getHostMetrics: (hostid: string, timeFrom?: number, timeTo?: number) => {
    const params = new URLSearchParams()
    if (timeFrom) params.append('time_from', timeFrom.toString())
    if (timeTo) params.append('time_to', timeTo.toString())
    return api.get(`/zabbix/metrics/${hostid}${params.toString() ? `?${params}` : ''}`)
  },
  getItemHistory: (hostid: string, itemid: string, timeFrom?: number, timeTo?: number, limit?: number) => {
    const params = new URLSearchParams()
    if (timeFrom) params.append('time_from', timeFrom.toString())
    if (timeTo) params.append('time_to', timeTo.toString())
    if (limit) params.append('limit', limit.toString())
    return api.get(`/zabbix/metrics/${hostid}/item/${itemid}${params.toString() ? `?${params}` : ''}`)
  },
}

// Metrics (PostgreSQL)
export const metricsAPI = {
  getPostgresMetrics: (deviceId: string, params?: {
    metric_name?: string
    time_from?: number
    time_to?: number
    limit?: number
    ...
    if (params?.time_from) query.append('time_from', params.time_from.toString())
    if (params?.time_to) query.append('time_to', params.time_to.toString())
    if (params?.limit) query.append('limit', params.limit.toString())
    return api.get(`/metrics/postgres/${deviceId}${query.toString() ? `?${query}` : ''}`)
  },
  getMetricsByIP: (deviceIp: string, params?: {
    metric_name?: string
    time_from?: number
    time_to?: number
    limit?: number
    ...
    if (params?.time_to) query.append('time_to', params.time_to.toString())
    if (params?.limit) query.append('limit', params.limit.toString())
    return api.get(`/metrics/postgres/devices/${deviceIp}${query.toString() ? `?${query}` : ''}`)
  },
  listAvailableMetrics: (deviceId?: string) => {
    const query = deviceId ? `?device_id=${deviceId}` : ''
    return api.get(`/metrics/postgres/list${query}`)
  },
}
